...
namespace pe {
	...
	static void onFrameSizeCallback(GLWindow* dwindow, int width, int height) {
	...
	Renderer::Renderer(const Descriptor& descriptor) noexcept {
		...
		mWindow = GLWindow::create(this, mWidth, mHeight);
		mWindow->setFrameSizeCallBack(onFrameSizeCallback);

		mInfos = GLInfo::create();
		mRenderList = GLRenderList::create();
		mAttributes = GLAttributes::create();
		mState = GLState::create();
		mBindingStates = GLBindingStates::create(mAttributes);
		mGeometries = GLGeometries::create(mAttributes, mInfos, mBindingStates);
		mObjects = GLObjects::create(mGeometries, mAttributes, mInfos);
		mPrograms = GLPrograms::create();
		mMaterials = GLMaterials::create(mPrograms);
		mBackground = GLBackground::create(this, mObjects);
		mRenderState = GLRenderState::create();
		mRenderTargets = GLRenderTargets::create();
		mTextures = GLTextures::create(mInfos, mRenderTargets);
		mShadowMap = GLShadowMap::create(this, mObjects, mState);
	...
	void Renderer::renderScene(
		const GLRenderList::Ptr& currentRenderList,
	...
	GLProgram::Ptr Renderer::setProgram(
		const Camera::Ptr& camera,
		const Scene::Ptr& scene,
		const Geometry::Ptr& geometry,
		const Material::Ptr& material,
		...
		GLMaterials::refreshMaterialUniforms(uniforms, material);
	...
	GLProgram::Ptr Renderer::getProgram(
		const Material::Ptr& material,
		const Scene::Ptr& scene,
		const RenderableObject::Ptr& object
	) noexcept {
	...
	void Renderer::setMouseMoveCallBack(const GLWindow::MouseMoveCallback& callback) noexcept {
		mWindow->setMouseMoveCallBack(callback);
	}

	void Renderer::setMouseActionCallback(const GLWindow::MouseActionCallback& callback) noexcept {
		mWindow->setMouseActionCallback(callback);
	}

	void Renderer::setKeyboardActionCallBack(const GLWindow::KeyboardActionCallback& callback) noexcept {
