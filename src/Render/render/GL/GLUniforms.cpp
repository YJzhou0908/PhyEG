...
#define UPLOAD_ARRAY(TYPE, VALUE) \
	...
	void SingleUniform::setValue(
		const std::any& value,
		const GLTextures::Ptr& textures,
		const std::shared_ptr<GLUniforms>& driverUniforms)
	{
	...
	GLUniforms::GLUniforms(const GLint& program) noexcept : UniformContainer() {
			...
			while (true) {
				if (std::regex_search(text, result, reg)) {
					...
					if (subscript.empty() || (subscript == "[" && matchEnd + 2 == text.length())) {
						...
						container = next.get(); // container指向结构体
					}

					text = result.suffix().str(); // 获取"."之后的剩余部分
				}
				else {
					break;
				}
	...
	GLUniforms::~GLUniforms() noexcept {}

	void GLUniforms::upload(UniformHandleMap& uniformHandleMap, const GLTextures::Ptr& textures) {
	...
	void GLUniforms::addUniform(UniformContainer* container, const UniformBase::Ptr& uniformObject) {
		container->mUniformMap.insert(std::make_pair(uniformObject->mID, uniformObject));
	}

	void GLUniforms::setTextureSlot(const GLint& location, GLuint slot) noexcept {
		mTextureSlots.insert(std::make_pair(location, slot));
	}

	GLint GLUniforms::getTextureSlot(const GLint& location) noexcept {
	...
	void GLUniforms::setTextureArraySlot(const GLint& location, std::vector<GLint> slots) noexcept {
		mTextureArraySlots.insert(std::make_pair(location, slots));
	}

	std::vector<GLint> GLUniforms::getTextureArraySlot(const GLint& location) noexcept {
	...
	std::vector<GLint> GLUniforms::allocateTextureUnits(const int& n) {
