...
namespace pe {
	...
	GLShadowMap::GLShadowMap(Renderer* renderer, const std::shared_ptr<GLObjects>& objects, const std::shared_ptr<GLState>& state) noexcept {
		mRenderer = renderer;
		mObjects = objects;
		mState = state;
	}

	GLShadowMap::~GLShadowMap() noexcept {}
	...
	//
	void GLShadowMap::render(const std::shared_ptr<GLRenderState>& renderState, const Scene::Ptr& scene, const Camera::Ptr& camera) noexcept {
		if (!mEnabled) return;
