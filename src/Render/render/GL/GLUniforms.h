...
namespace pe {
	...
	public:
		...
		void setValue(
			const std::any& value,
			const GLTextures::Ptr& textures,
			const std::shared_ptr<GLUniforms>& driverUniforms) override;
	...
	class GLUniforms :public UniformContainer, public std::enable_shared_from_this<GLUniforms> {
	public:
		using Ptr = std::shared_ptr<GLUniforms>;
		static Ptr create(const GLint& program) { return std::make_shared<GLUniforms>(program); }

		GLUniforms(const GLint& program) noexcept;

		~GLUniforms() noexcept;

		// uniform变量名 映射到 container里面
		...
		void upload(UniformHandleMap& uniformHandleMap, const GLTextures::Ptr& textures);
