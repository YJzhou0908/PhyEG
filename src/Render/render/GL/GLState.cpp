...
namespace pe {
	...
	GLState::GLState() noexcept {
		glEnable(GL_MULTISAMPLE);
	}

	GLState::~GLState() noexcept {}

	void GLState::viewport(const glm::vec4& viewport) noexcept {
	...
	bool GLState::useProgram(GLuint program) noexcept {
	...
	void GLState::setMaterial(const Material::Ptr& material) noexcept {
		//对于双面渲染，有两种方案
		// 1 在绘制背面的时候，进行法线的反转
	...
	void GLState::setBlendingInternal(
		bool				transparent,
		BlendingFactor		blendSrc,
		BlendingFactor		blendDst,
		BlendingEquation	blendEquation,
	...
	void  GLState::bindFrameBuffer(const GLuint& frameBuffer) noexcept {
	...
	void GLState::setClearColor(float r, float g, float b, float a) noexcept {
	...
	glm::vec4 GLState::getClearColor() const noexcept {
